...
#let new-section-slide(
  ...
  numbered: true,
  sidebar-fill: auto,
  sidebar-content: image("../themes/logo2.svg", width: 40%),
  sidebar-width: 20%,
  body,
            ...
            if all-headings.len() > 0 {
              ...
              .map(pair => {
                ...
                block(width: 100%, inset: (bottom: spacing), {
                  // 添加计数器和标题文本
                  text(fill: self.colors.primary, weight: "medium", str(count) + ". " + title-text)
                })
              })
...
#let university-theme(
  ...
  footer-b: self => {
    ...
    let cell(fill, content, location-target: none) = {
      ...
      // 移除了超链接，直接使用文本内容
      let final-content = cell-content
